...
. En la pantalla de VMware, haga clic en *vSphere Web Client (HTML5)* y, a continuación, inicie sesión en la página *VMware vCenter Single Sign-On*.
. Haga clic con el botón derecho del ratón en cualquier objeto de inventario que sea un objeto principal válido de una máquina virtual, como un centro de datos, una carpeta, un clúster o un host, y seleccione *implementar plantilla OVF* para iniciar el asistente.
